...
async def evaluate(args) -> int:
    ...
    try:
        ...
        async with AsyncExitStack() as stack:
            ...
            service = RecommendationService(database, providers)
            if args.track_id is None:
                result = await service.recommend_for_user(args.telegram_user_id, args.limit, random_seed=args.seed)
            else:
                result = await service.recommend_similar_to_track(args.telegram_user_id, args.track_id, args.limit, random_seed=args.seed)
            if result.status != "ok":
            ...
            for index, row in enumerate(result.recommendations, 1):
                report(f"{index}. {short_metadata(row.artist)} — {short_metadata(row.title)} | score={row.score:.4f} | {row.reason} | "
                       f"track_id={row.track_id} | sources={','.join(row.sources)} | exploration={row.exploration}")
...
def run() -> int:
    ...
    parser.add_argument("--track-id", type=int, help="More Like This: canonical track ID; omit for For You")
    ...
    if args.track_id is not None and args.track_id <= 0:
        parser.error("Track ID must be positive")
    configure_logging()
    try:
